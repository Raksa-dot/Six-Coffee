...
export default function Login({ onLogin }) {
  ...
  function handleLogin(event) {
    ...
    onLogin({
      nama: form.email.split("@")[0] || "User MindMate",
      email: form.email,
    })
    navigate("/dashboard")
  }
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-gradient-to-br from-[#f1e7ff] via-[#fbf8ff] to-[#dcfaff] px-4 py-10">
      ...
      <div className="relative z-10 w-full max-w-md mindmate-card rounded-3xl p-7 sm:p-8 backdrop-blur">
        ...
        <div className="text-center">
          <h1 className="text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Masuk Akun</h1>
          <p className="mx-auto mt-2 max-w-xs text-sm leading-5 text-slate-500">
            Masuk untuk melanjutkan ke dashboard MindMate AI.
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="mt-7 grid gap-4">
          ...
          <Input name="password" value={form.password} onChange={ubahForm} icon={<Lock size={17} />} label="Password" type="password" placeholder="Masukkan password" />
        ...
        <p className="mt-6 text-center text-xs text-slate-500">
          Belum punya akun?{" "}
          <Link to="/daftar" className="font-black text-violet-700 transition hover:text-violet-700 hover:underline">
            Daftar
          </Link>
        </p>
      </div>
